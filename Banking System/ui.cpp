...
static void draw_header() {
    ...
    const char* title = "Concurrent Banking Transaction System";
...
static void draw_accounts() {
    ...
    // Updated Header: Removed "Sem", Added "Waiting VIPs (W)"
    wattron(win_accounts, A_BOLD | A_UNDERLINE);
    mvwprintw(win_accounts, 1, 2, "%-6s %-10s %10s %-5s %-8s %s",
              "AccNo","Name","Balance","VIP","Status","Waiting");
    ...
    for (int i = 0; i < num_accounts && i < rows - 10; i++) {
        ...
        // NEW LOGIC: Use is_busy instead of sem_getvalue
        bool is_mine = (logged_in_acc != 0 && a.acc_number == logged_in_acc);

        // Highlight logic based on your proposal
        if (a.frozen) {
            wattron(win_accounts, COLOR_PAIR(CP_FROZEN) | A_BOLD);
        } else if (a.waiting_vips > 0) {
            // Blink if a VIP is waiting (Visualizes the Priority Queue)
            wattron(win_accounts, COLOR_PAIR(CP_VIP) | A_BOLD | A_BLINK);
        } else if (a.is_busy) {
        ...
        const char* status_str = a.frozen      ? "FROZEN"
                               : a.waiting_vips > 0 ? "PRIORITY" 
                               : a.is_busy     ? "LOCKED"
                               : is_mine       ? "<<YOU>>"
                                               : "ACTIVE";

        // Updated print: Displays waiting_vips counter
        mvwprintw(win_accounts, 2 + i, 2,
                  "%-6ld %-10s %10.2f %-5s %-8s %d",
                  a.acc_number, a.name.substr(0,10).c_str(),
                  a.balance, (a.is_vip ? "VIP" : "-"), 
                  status_str, a.waiting_vips);
    ...
    // Bar chart (unchanged logic, just ensuring consistency)
    int chart_top = num_accounts + 4;
    if (chart_top < rows - 4) {
        wattron(win_accounts, A_BOLD);
        mvwprintw(win_accounts, chart_top, 2, "Balance Consistency Chart:");
        ...
        for (int i = 0; i < num_accounts && (chart_top + 1 + i) < rows - 1; i++) {
            ...
            if (accounts[i].frozen) wattron(win_accounts, COLOR_PAIR(CP_FAIL));
            else if (accounts[i].is_vip) wattron(win_accounts, COLOR_PAIR(CP_VIP));
            else wattron(win_accounts, COLOR_PAIR(CP_CHART));
            ...
            wattroff(win_accounts, COLOR_PAIR(CP_FAIL)|COLOR_PAIR(CP_VIP)|COLOR_PAIR(CP_CHART));
        }
    }
...
static void draw_status() {     
     ...
     // Limit status message length to prevent UI box break   
     mvwprintw(win_status, 1, 2, "Status: %.60s", status_msg);  
     int cols = getmaxx(win_status);    
     char hint[128];     
     if (logged_in_acc == 0)      
        snprintf(hint, sizeof(hint), "[N] New Account [T] Transfer [W] Withdraw [D] Deposit [Q] Logout");   
     else 
        snprintf(hint, sizeof(hint), "[T] Transfer [W] Withdraw [D] Deposit [Q] Logout");     
     int hlen = (int)strlen(hint);     
     int hx = cols - hlen - 2;     
     if (hx > 65) { // Only draw if space permits
...
void ui_login_screen() {
    ...
    int wh=12, ww=60, wy=(rows-wh)/2, wx=(cols-ww)/2;
    ...
    mvwprintw(lw, 3, 2, "Enter Account Number to log in.");
    mvwprintw(lw, 4, 2, "Enter 0 for Manager, or -1 to EXIT SYSTEM.");
    
    wattron(lw, COLOR_PAIR(CP_MGRINF));
    // Changed to full words here:
    mvwprintw(lw, 6, 2, " Manager : [New Account] [Transfer] [Withdraw] [Deposit]");
    mvwprintw(lw, 7, 2, " User    : [Transfer] [Withdraw] [Deposit] [Logout]");
    wattroff(lw, COLOR_PAIR(CP_MGRINF));

    mvwprintw(lw, 9, 2, "Account ID: ");
    wrefresh(lw);
    popup_begin();
    char input[24] = {};
...
void ui_new_account_popup() {
    ...
    if (new_acc > 0) {
        ...
        mvwprintw(pw, 4, 2, "Account No  : %ld", new_acc);
...
void ui_withdraw_popup() {
    ...
    char ok_msg[128], fail_msg[64];
    snprintf(ok_msg,   sizeof(ok_msg),  "Withdraw $%.2f from Acc %ld", amount, target_acc);
    snprintf(fail_msg, sizeof(fail_msg), "%s", txStatusStr(s));
    if (s == TX_SUCCESS) ui_set_status(ok_msg);
    else                 ui_set_status(fail_msg);
